...
% This report first covers the repository structure. Then the implementation of dual numbers is discussed along with how the package may be configured with \texttt{pyproject.toml}, installed with \texttt{pip}, and tested using \texttt{pytest}. The documentation of the package with \texttt{Sphinx} is also discussed. Then, Cythonization is covered, along with comparisons made between the Cythonized and pure Python versions of the package. Finally, how the Cythonized package may be installed from wheels is discussed.
...
\lstinputlisting[caption={Contents of the pyproject.toml file.}]{../pyproject.toml}
\newpage
...
\lstinputlisting[caption={Dual class initialisation in Python file},language=python,linerange={5,14,22, 23}]{../dual_autodiff/dual.py}
...
\lstinputlisting[caption={setup.py}, language=Python]{../dual_autodiff_x/setup.py}
